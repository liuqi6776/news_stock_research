...
def preload_and_save_trades():
    print("Step 1: Getting dates...")
    dates = get_all_dates()
...
def test_take_profit_on_preloaded(trades_df):
...
if __name__ == "__main__":
    ...
    if os.path.exists(trades_csv):
        print(f"发现预加载文件已存在: {trades_csv}")
        trades_df = pd.read_csv(trades_csv)
        print(f"已加载 {len(trades_df)} 条交易数据")
    else:
        print("Preloading trades...")
        trades_df = preload_and_save_trades()
    
    if trades_df is not None:
        test_take_profit_on_preloaded(trades_df)
    else:
        print("trades_df is None, exiting.")
