"""
年度滚动回测框架 (Walk-Forward Analysis) - V3
整合doubao策略特征: hot_rank, chip, news等
先用年级别的rolling train/test，确认有效feature后再切换到月级别
"""
import pandas as pd
import numpy as np
import os
...
class WalkForwardBacktestV3:
    """年度滚动回测类 - V3（整合doubao特征）"""
    ...
    def save_model(self, model, scaler, feature_cols, period_name):
        ...
        model_filename = f"model_v3_{period_name}_{timestamp}.pkl"
        scaler_filename = f"scaler_v3_{period_name}_{timestamp}.pkl"
        feature_filename = f"features_v3_{period_name}_{timestamp}.json"
    ...
    def run_walk_forward(self, start_date='20200101', end_date='20260331',
        ...
        """
        print("=" * 80)
        print("年度滚动回测 (Walk-Forward Analysis) - V3")
        print("=" * 80)
        ...
        for i in range(train_years, len(years), test_years):
            ...
            if save_predictions:
                ...
                if not predictions_df.empty:
                    pred_file = os.path.join(self.model_dir, f"predictions_v3_{period_name}.csv")
                    ...
                    meta_file = os.path.join(self.model_dir, f"meta_v3_{period_name}.json")
    ...
    def save_results(self, results, all_equity, all_trades):
        ...
        if results:
            ...
            results_file = os.path.join(self.output_dir, f'yearly_results_v3_{timestamp}.csv')
        ...
        if all_equity:
            ...
            equity_file = os.path.join(self.output_dir, f'equity_curve_v3_{timestamp}.csv')
        ...
        if all_trades:
            ...
            trades_file = os.path.join(self.output_dir, f'all_trades_v3_{timestamp}.csv')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='年度滚动回测V3')
    ...
    backtest = WalkForwardBacktestV3()
