"""
完整回测框架 V8
整合：价格特征 + 技术因子 + 缠论特征 + news_major1 + ths_rank + 筹码数据 + 市值数据
训练：Rolling一年更新
测试：月度滚动
"""
import pandas as pd
...
class WalkForwardAllFeaturesV8:
    """完整特征回测类 V8"""
    ...
    def train_model(self, train_df: pd.DataFrame, feature_cols: List[str], 
                   params: dict = None) -> Tuple[xgb.XGBClassifier, StandardScaler]:
        """训练模型"""
    ...
    def run_baseline(self, start_date='20230101', end_date='20241231',
        ...
        for i in range(train_months, len(months), test_months):
            ...
            model, scaler = self.train_model(train_df, feature_cols, baseline_params)
        ...
        if results:
            ...
            results_df.to_csv(os.path.join(self.output_dir, f'baseline_v8_results_{timestamp}.csv'), index=False)
            
            if all_equity:
                equity_df = pd.concat(all_equity, ignore_index=True)
                equity_df.to_csv(os.path.join(self.output_dir, f'baseline_v8_equity_{timestamp}.csv'), index=False)
            
            if all_trades:
                trades_df = pd.concat(all_trades, ignore_index=True)
                trades_df.to_csv(os.path.join(self.output_dir, f'baseline_v8_trades_{timestamp}.csv'), index=False)
            ...
            if all_feature_importance:
                ...
                feat_imp_df.to_csv(os.path.join(self.output_dir, f'baseline_v8_feature_importance_{timestamp}.csv'), index=False)
    ...
    def run_tuned(self, start_date='20230101', end_date='20241231',
                 train_months=12, test_months=1, min_prob=0.55):
        """运行微调后的模型"""
        print("=" * 80)
        print("微调模型 - 优化参数")
        ...
        for i in range(train_months, len(months), test_months):
            ...
            # 训练微调模型
            print("训练微调模型...")
            model, scaler = self.train_model(train_df, feature_cols, tuned_params)
        ...
        if results:
            results_df = pd.DataFrame(results)
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            results_df.to_csv(os.path.join(self.output_dir, f'tuned_v8_results_{timestamp}.csv'), index=False)
            
            if all_equity:
                equity_df = pd.concat(all_equity, ignore_index=True)
                equity_df.to_csv(os.path.join(self.output_dir, f'tuned_v8_equity_{timestamp}.csv'), index=False)
            
            if all_trades:
                trades_df = pd.concat(all_trades, ignore_index=True)
                trades_df.to_csv(os.path.join(self.output_dir, f'tuned_v8_trades_{timestamp}.csv'), index=False)
...
if __name__ == "__main__":
    # 运行Baseline
    bt = WalkForwardAllFeaturesV8()
    
    print("\n" + "="*80)
    print("第一步：运行Baseline模型")
    print("="*80)
