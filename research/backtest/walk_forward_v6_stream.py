...
def calc_features_for_stock(hist_df):
    """对单只股票的历史数据计算所有特征，返回最新一天的特征 - 全向量化版本"""
    if len(hist_df) < 30:
        return None
...
class BacktestV6Stream:
    ...
    def get_day_data(self, date):
        ...
        # 缓存管理
        self.price_cache[date] = df
        self.cache_dates.append(date)
        if len(self.cache_dates) > 80:
            old = self.cache_dates.pop(0)
            if old in self.price_cache:
                del self.price_cache[old]

        return df

    def get_stock_history(self, ts_code, end_date, n_days=70):
        """获取单只股票的历史数据"""
        try:
    ...
    def build_training_sample(self, d_curr, d_t1, d_t2):
        """为单个交易日构建训练样本 - 只处理活跃股票"""
        df_t = self.get_day_data(d_curr)
        if df_t is None:
            return pd.DataFrame()

        df_t1 = self.get_day_data(d_t1)
        df_t2 = self.get_day_data(d_t2)
        if df_t1 is None or df_t2 is None:
            return pd.DataFrame()

        # 只处理当日成交量前60%的活跃股票，减少计算量
        ...
        for ts_code in active_stocks:
            # 获取历史数据计算特征
            hist = self.get_stock_history(ts_code, d_curr, n_days=70)
            if hist is None:
                continue

            features = calc_features_for_stock(hist)
            if features is None:
                continue
            ...
            sample = {
                ...
                'hot_rank_pct': features['hot_rank_pct'],
                'chip_concentration': features['chip_concentration'],
                'winner_rate': features['winner_rate'],
                'circ_mv': features['circ_mv'],
                'turnover_rate': features['turnover_rate'],
                'volume_ratio': features['volume_ratio'],
                'top_fractal': features['top_fractal'],
                'bottom_fractal': features['bottom_fractal'],
                'bi_direction': features['bi_direction'],
                'zhongshu': features['zhongshu'],
                'zhongshu_strength': features['zhongshu_strength'],
                'top_divergence': features['top_divergence'],
                'bottom_divergence': features['bottom_divergence'],
                'mom_5d': features['mom_5d'],
                'mom_10d': features['mom_10d'],
                'mom_20d': features['mom_20d'],
                'vol_5d': features['vol_5d'],
                'vol_10d': features['vol_10d'],
                'vol_20d': features['vol_20d'],
                'vol_ratio_5d': features['vol_ratio_5d'],
                'vol_ratio_10d': features['vol_ratio_10d'],
                'vol_ratio_20d': features['vol_ratio_20d'],
                'amount_ratio_5d': features['amount_ratio_5d'],
                'amount_ratio_10d': features['amount_ratio_10d'],
                'ma_dist_5d': features['ma_dist_5d'],
                'ma_dist_10d': features['ma_dist_10d'],
                'ma_dist_20d': features['ma_dist_20d'],
                'ma_dist_60d': features['ma_dist_60d'],
                'ma_bull': features['ma_bull'],
                'ma_bear': features['ma_bear'],
                'rsi_6d': features['rsi_6d'],
                'rsi_12d': features['rsi_12d'],
                'rsi_24d': features['rsi_24d'],
                'macd': features['macd'],
                'macd_hist': features['macd_hist'],
                'bb_width': features['bb_width'],
                'bb_position': features['bb_position'],
                'kdj_k': features['kdj_k'],
                'kdj_d': features['kdj_d'],
                'kdj_j': features['kdj_j'],
                'news_market_impact': features['news_market_impact'],
                'news_stock_impact': features['news_stock_impact'],
            }
            samples.append(sample)

        return pd.DataFrame(samples)

    def prepare_data(self, dates):
        """准备训练数据 - 逐日流式处理"""
        ...
        for i in range(len(dates) - 2):
            ...
            sample = self.build_training_sample(d_curr, d_t1, d_t2)
            if not sample.empty:
                all_samples.append(sample)

            if (i + 1) % 50 == 0:
                print(f"  已处理 {i+1}/{len(dates)-2} 天, 样本: {sum(len(s) for s in all_samples)}")
                gc.collect()

        if not all_samples:
            return pd.DataFrame(), []

        full = pd.concat(all_samples, ignore_index=True)
    ...
    def predict_day(self, model, scaler, features, date):
        ...
        preds = []
        for ts_code in active_stocks:
            hist = self.get_stock_history(ts_code, date, n_days=70)
            if hist is None:
                continue
            feat = calc_features_for_stock(hist)
            if feat is None:
                continue
            preds.append({
                'ts_code': ts_code,
    ...
    def backtest(self, predictions_df, test_dates, min_prob=0.55):
        ...
        for i in range(len(test_dates) - 2):
            ...
            day_df = self.get_day_data(d_curr)
            if day_df is None:
    ...
    def run(self, start_date='20200101', end_date='20260331', train_years=2, test_years=1, min_prob=0.55):
        ...
        for i in range(train_years, len(years), test_years):
            ...
            if all_preds:
                ...
                print("回测...")
                bt = self.backtest(pred_df, test_dates, min_prob)
            else:
...
if __name__ == '__main__':
    bt = BacktestV6Stream()
    bt.run(start_date='20210101', end_date='20260331', train_years=1, test_years=1, min_prob=0.55)
