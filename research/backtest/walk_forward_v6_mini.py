...
class BacktestV6Mini:
    ...
    def get_stock_hist(self, ts_code, end_date, n_days=70):
        try:
    ...
    def build_sample(self, d_curr, d_t1, d_t2):
        df_t = self.get_day(d_curr)
        if df_t is None:
            return pd.DataFrame()

        df_t1 = self.get_day(d_t1)
        df_t2 = self.get_day(d_t2)
        if df_t1 is None or df_t2 is None:
            return pd.DataFrame()

        samples = []
        # 限制股票数量，避免处理太慢
        stock_list = df_t['ts_code'].unique()[:500]  # 只处理前500只

        for ts_code in stock_list:
            hist = self.get_stock_hist(ts_code, d_curr, n_days=70)
            if hist is None:
                continue
    ...
    def prepare_data(self, dates):
        ...
        for i in range(len(dates) - 2):
            sample = self.build_sample(dates[i], dates[i+1], dates[i+2])
            if not sample.empty:
                all_samples.append(sample)

            if (i + 1) % 100 == 0:
                total = sum(len(s) for s in all_samples)
                print(f"  已处理 {i+1}/{len(dates)-2} 天, 样本: {total}")
                gc.collect()
    ...
    def predict_day(self, model, scaler, features, date):
        df = self.get_day(date)
        if df is None:
            return pd.DataFrame()

        preds = []
        for ts_code in df['ts_code'].unique()[:500]:  # 只处理前500只
            hist = self.get_stock_hist(ts_code, date, n_days=70)
            if hist is None:
                continue
            feat = calc_features(hist)
            if feat is None:
    ...
    def run(self, start_date='20200101', end_date='20260331', train_years=2, test_years=1, min_prob=0.55):
        ...
        for i in range(train_years, len(years), test_years):
            ...
            results.append({
                ...
                'test_sharpe': bt['test_sharpe'], 'test_max_dd': bt['max_dd'],
