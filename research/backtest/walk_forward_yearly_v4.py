"""
年度滚动回测框架 (Walk-Forward Analysis) - V4
优化版本：
1. 添加更多有效特征（动量、波动率、成交量等）
2. 使用更稳健的模型参数
3. 改进交易逻辑
4. 添加特征选择
"""
...
class WalkForwardBacktestV4:
    ...
    def load_daily_features(self, date: str) -> pd.DataFrame:
        ...
        # 计算价格特征
        price_df['price_position'] = (price_df['close'] - price_df['low']) / (price_df['high'] - price_df['low'] + 1e-8)
        ...
        if os.path.exists(p_other):
            try:
                other_df = pd.read_parquet(p_other, columns=['ts_code', 'circ_mv', 'turnover_rate', 'volume_ratio', 'pe', 'pb'])
    ...
    def select_features(self, X, y, feature_cols, k=15):
        """特征选择"""
        if X.shape[1] <= k:
    ...
    def train_model(self, train_df: pd.DataFrame, feature_cols: List[str]) -> Tuple[xgb.XGBClassifier, StandardScaler, List[str]]:
        ...
        # 特征选择
        selected_features, X_selected = self.select_features(X, y, feature_cols, k=min(15, len(feature_cols)))
    ...
    def save_model(self, model, scaler, feature_cols, period_name):
        ...
        model_filename = f"model_v4_{period_name}_{timestamp}.pkl"
        scaler_filename = f"scaler_v4_{period_name}_{timestamp}.pkl"
        feature_filename = f"features_v4_{period_name}_{timestamp}.json"
    ...
    def run_walk_forward(self, start_date='20200101', end_date='20260331',
                        train_years=2, test_years=1, min_prob=0.6,
        ...
        """
        print("=" * 80)
        print("年度滚动回测 (Walk-Forward Analysis) - V4 (优化版)")
        print("=" * 80)
        ...
        for i in range(train_years, len(years), test_years):
            ...
            if save_predictions:
                ...
                if not predictions_df.empty:
                    pred_file = os.path.join(self.model_dir, f"predictions_v4_{period_name}.csv")
                    ...
                    meta_file = os.path.join(self.model_dir, f"meta_v4_{period_name}.json")
    ...
    def save_results(self, results, all_equity, all_trades):
        ...
        if results:
            ...
            results_file = os.path.join(self.output_dir, f'yearly_results_v4_{timestamp}.csv')
        ...
        if all_equity:
            ...
            equity_file = os.path.join(self.output_dir, f'equity_curve_v4_{timestamp}.csv')
        ...
        if all_trades:
            ...
            trades_file = os.path.join(self.output_dir, f'all_trades_v4_{timestamp}.csv')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='年度滚动回测V4')
    ...
    backtest = WalkForwardBacktestV4()
