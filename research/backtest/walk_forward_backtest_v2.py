"""
月度滚动回测框架 (Walk-Forward Analysis) - 优化版
训练期: 2年 (24个月)
测试期: 1个月
步长: 1个月
总回测期: 2023-2026 (至少3年)
...
class DataLoader:
    ...
    def __init__(self, data_dir: str = r'D:\iquant_data\data_v2'):
        ...
        self.price_data = {}
        print("预加载价格数据...")
        for date in tqdm(self.all_dates[:500]):  # 限制加载数量避免内存溢出
            try:
                df = pd.read_parquet(os.path.join(self.price_dir, f"{date}.parquet"))
                df['trade_date'] = date
                self.price_data[date] = df
            except Exception as e:
                print(f"加载 {date} 失败: {e}")
        
        print(f"已加载 {len(self.price_data)} 天的数据")
    ...
    def get_date_data(self, date_str: str) -> Optional[pd.DataFrame]:
        """获取某一天的数据"""
        return self.price_data.get(date_str)
...
class FeatureEngineer:
    ...
    @staticmethod
    def calculate_features_for_period(df: pd.DataFrame) -> pd.DataFrame:
        """
        为一段时间的数据计算因子
        df需要包含: ts_code, trade_date, open, high, low, close, vol
        """
        if len(df) == 0:
            return df
    ...
    @staticmethod
    def prepare_training_features(loader: DataLoader, 
                                  dates: List[str],
        ...
        for i in range(len(dates) - 2):
            ...
            # 获取数据
            df_t = loader.get_date_data(d_curr)
            df_t1 = loader.get_date_data(d_t1)
            df_t2 = loader.get_date_data(d_t2)
            
            if df_t is None or df_t1 is None or df_t2 is None:
                continue
            
            # 过滤主板
            df_t = df_t[df_t['ts_code'].apply(loader.is_main_board)]
...
class WalkForwardBacktest:
    ...
    def backtest_month(self,
        ...
        for i in range(len(test_dates) - 2):
            ...
            # 加载T日特征
            df_t = self.loader.get_date_data(d_curr)
            if df_t is None or len(df_t) == 0:
                daily_nav.append({'date': d_t2, 'nav': capital})
                continue
            
            # 过滤主板
            df_t = df_t[df_t['ts_code'].apply(self.loader.is_main_board)]
            ...
            # 获取T+1和T+2价格
            df_t1 = self.loader.get_date_data(d_t1)
            df_t2 = self.loader.get_date_data(d_t2)
            ...
            # 获取涨跌停幅度
            limit_pct = self.loader.get_limit_pct(ts_code)
    ...
    def run_walk_forward(self,
        ...
        # 过滤日期范围
        dates = [d for d in self.loader.all_dates if start_date <= d <= end_date]
        ...
        for i in range(train_months, len(months), test_months):
            ...
            # 准备训练数据
            print("准备训练数据...")
            train_df, feature_cols = FeatureEngineer.prepare_training_features(
                self.loader, train_dates
            )
