"""
年度滚动回测框架 (Walk-Forward Analysis) - V5
简化优化版本：
1. 使用核心特征（价格、筹码、热度、新闻）
2. 改进标签定义
3. 优化交易逻辑
4. 添加风险控制
"""
...
class WalkForwardBacktestV5:
    ...
    def save_model(self, model, scaler, feature_cols, period_name):
        ...
        model_filename = f"model_v5_{period_name}_{timestamp}.pkl"
        scaler_filename = f"scaler_v5_{period_name}_{timestamp}.pkl"
        feature_filename = f"features_v5_{period_name}_{timestamp}.json"
    ...
    def run_walk_forward(self, start_date='20200101', end_date='20260331',
        ...
        """
        print("=" * 80)
        print("年度滚动回测 (Walk-Forward Analysis) - V5 (简化优化版)")
        print("=" * 80)
        ...
        for i in range(train_years, len(years), test_years):
            ...
            if save_predictions:
                ...
                if not predictions_df.empty:
                    pred_file = os.path.join(self.model_dir, f"predictions_v5_{period_name}.csv")
                    ...
                    meta_file = os.path.join(self.model_dir, f"meta_v5_{period_name}.json")
    ...
    def save_results(self, results, all_equity, all_trades):
        ...
        if results:
            ...
            results_file = os.path.join(self.output_dir, f'yearly_results_v5_{timestamp}.csv')
        ...
        if all_equity:
            ...
            equity_file = os.path.join(self.output_dir, f'equity_curve_v5_{timestamp}.csv')
        ...
        if all_trades:
            ...
            trades_file = os.path.join(self.output_dir, f'all_trades_v5_{timestamp}.csv')
...
def main():
    ...
    parser = argparse.ArgumentParser(description='年度滚动回测V5')
    ...
    backtest = WalkForwardBacktestV5()
