"""
完整回测框架 V7
整合：价格特征 + 技术因子 + news_major1 + ths_rank + 筹码数据 + 市值数据
训练：Rolling一年更新
测试：月度滚动
"""
import pandas as pd
...
class WalkForwardCompleteV7:
    ...
    def run_walk_forward(self, start_date='20230101', end_date='20260331',
        ...
        """运行月度滚动回测"""
        print("=" * 80)
        print("完整回测框架 V7 (含news_major1 + ths_rank + 筹码 + 市值)")
        print("=" * 80)
        ...
        if results:
            ...
            results_df.to_csv(os.path.join(self.output_dir, f'complete_v7_results_{timestamp}.csv'), index=False)
            
            if all_equity:
                equity_df = pd.concat(all_equity, ignore_index=True)
                equity_df.to_csv(os.path.join(self.output_dir, f'complete_v7_equity_{timestamp}.csv'), index=False)
            
            if all_trades:
                trades_df = pd.concat(all_trades, ignore_index=True)
                trades_df.to_csv(os.path.join(self.output_dir, f'complete_v7_trades_{timestamp}.csv'), index=False)
