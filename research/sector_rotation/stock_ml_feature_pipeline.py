...
输出:
  - research/sector_rotation/stock_ml_panel_72m.parquet
    列: [trade_date(月首调仓日), ts_code, industry, is_traditional,
         ret_1m, ivol, momentum_5/10/20/60, volatility_5/10/20,
         alpha_006/009/012/023, roe, or_yoy, netprofit_yoy, fwd_20]
...
OUT = os.path.join(ROOT, "research", "sector_rotation", "stock_ml_panel_72m.parquet")
...
def _roll_conc(df):
    ...
    for i in range(19, n):
        sl = slice(i-19, i+1)
        out[i] = _chip_conc(close_arr[sl], vol_arr[sl], 20)
...
# 快照范围 2020-01 ~ 2025-12 (2020-01 无可用成分快照)
