# -*- coding: utf-8 -*-
"""只跑 LSTM + C8 baseline (跳过 FTT, CPU 上太慢)"""
...
def _train_lstm(model, Xtr, ytr, Xv, yv, epochs=30, lr=1e-3, wd=1e-2):
    ...
    for ep in range(epochs):
        ...
        if vl < best_vl: best_vl = vl; best_sd = {k:v.cpu().clone() for k,v in model.state_dict().items()}; pat = 0
        else:
            pat += 1
            if pat >= 8: break
        sched.step(vl)
    ...
    class _P:
        def predict(self, X):
            with torch.no_grad(): return model(torch.from_numpy(X.astype(np.float32)).to(DEV)).cpu().numpy().ravel()
    return _P()
...
# LSTM
print(f"\n>> LSTM (hidden=64, 1层双向, {NF}因子, 30 epoch)...")
def lstm_f(Xtr_3d, ytr, Xv_3d, yv, n_feat):
    m = LSTM_M(n_feat, hidden=64, n_layers=1, dropout=0.1).to(DEV)
    return _train_lstm(m, torch.from_numpy(Xtr_3d).to(DEV), ytr,
                          torch.from_numpy(Xv_3d).to(DEV), yv, epochs=30)
r = wfo(all_feats, lstm_f, need_scale=True, label="LSTM_64", is_3d=True)
