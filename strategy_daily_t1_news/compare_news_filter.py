"""
Comparison Backtest: News-Required vs All-Stocks Strategy
- Period: 2024-01-01 to 2026-03-26
- WFO: Quarterly model retraining (3-month steps)
- Strategy A: Only selects stocks with news_stock_impact > 0
- Strategy B: Selects any top-ranked stock (no news filter)
"""
import os
import sys
import pandas as pd
...
def run_one_leg(test_dates, model, feats, news_market_df, news_stock_sector_df,
                capital, require_news=False):
    """Single pass through all test days, returns equity list and final capital."""
    equity = []

    for i in tqdm(range(len(test_dates) - 1),
                  desc=f"  [{'News-Required' if require_news else 'All-Stocks'}]", leave=False):
        ...
        # === KEY DIFFERENCE ===
        if require_news:
            pool = df[df['news_stock_impact'] > 0]
        else:
            pool = df

        picks = pool[pool['prob'] > 0.8].sort_values('prob', ascending=False).head(3)
        if picks.empty:
            picks = pool.sort_values('prob', ascending=False).head(1)
        if picks.empty:
...
def run_comparison_backtest(start_date='20240101', end_date='20260326',
                             train_start='20220101', step_months=3):
    print("=== Strategy Comparison Backtest (Quarterly WFO) ===")
    ...
    capital_A   = initial_cap   # News-Required
    capital_B   = initial_cap   # All-Stocks
    equity_A, equity_B = [], []
    ...
    while current_test_start <= final_end_date:
        ...
        # Run both legs on the same data/model
        seg_A, capital_A = run_one_leg(test_dates, model, feats, news_mkt, news_stk,
                                        capital_A, require_news=True)
        seg_B, capital_B = run_one_leg(test_dates, model, feats, news_mkt, news_stk,
                                        capital_B, require_news=False)

        equity_A.extend(seg_A)
        equity_B.extend(seg_B)
    ...
    sa = stats(equity_A, capital_A)
    sb = stats(equity_B, capital_B)

    print("\n" + "="*60)
    print(f"{'Strategy':<28} {'Ann.Ret':>9} {'Sharpe':>8} {'MDD':>8} {'TotalRet':>10}")
    print("-"*60)
    print(f"{'News-Required (A)':<28} {sa.get('ann_ret', 0):>9.2%} {sa.get('sharpe', 0):>8.2f} {sa.get('mdd', 0):>8.2%} {sa.get('total_ret', 0):>10.2%}")
    print(f"{'All-Stocks    (B)':<28} {sb.get('ann_ret', 0):>9.2%} {sb.get('sharpe', 0):>8.2f} {sb.get('mdd', 0):>8.2%} {sb.get('total_ret', 0):>10.2%}")
    print("="*60)
    ...
    if equity_A:
        ...
        ax.plot(df_A['date'], df_A['nav'],
                label=f"A: News-Required | Ann: {sa.get('ann_ret',0):.1%} | Sharpe: {sa.get('sharpe',0):.2f}",
                color='royalblue', linewidth=1.8)

    if equity_B:
        df_B = pd.DataFrame(equity_B)
        ax.plot(df_B['date'], df_B['nav'],
                label=f"B: All-Stocks    | Ann: {sb.get('ann_ret',0):.1%} | Sharpe: {sb.get('sharpe',0):.2f}",
                color='tomato', linewidth=1.8, linestyle='--')

    ax.axhline(initial_cap, color='gray', linewidth=0.8, linestyle=':')
    ax.set_title("News-Required vs All-Stocks Strategy (2024-2026, Quarterly WFO)", fontsize=14)
    ax.set_xlabel("Date")
    ...
    out_path = os.path.join(out_dir, 'news_vs_all_comparison.png')
    plt.tight_layout()
