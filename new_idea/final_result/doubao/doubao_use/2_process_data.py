"""
Step 2: Download daily data from Tushare
- Price data (data_day1)
- Market cap data (other_day1)
- THS hot rank (ths_rank1)
- Chip distribution (cyq1)
"""
import os
import sys
import pandas as pd
...
def fetch_data_for_date(target_date):
    ...
    if not os.path.exists(p_chip):
        ...
        try:
            ...
            dfs = []
            for i, code in enumerate(tqdm(stock_list, desc="  筹码下载")):
                try:
                    cdf = pro.cyq_perf(ts_code=code, start_date=target_date, end_date=target_date)
                    if not cdf.empty:
                        dfs.append(cdf)
                except:
                    time.sleep(0.3)
            ...
            if dfs:
                ...
                print(f"  保存成功: {p_chip} ({len(final_chip)}只)")
            else:
                print(f"  警告: 未获取到筹码数据")
        except Exception as e:
            print(f"  下载筹码失败: {e}")
