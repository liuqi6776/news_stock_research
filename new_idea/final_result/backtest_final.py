...
def run_backtest(trades_df, all_dates_set):
...
def main():
    print("=" * 80)
    print(f"  FINAL BACKTEST (No Take Profit, circ_mv<={CIRC_MV_LIMIT})")
    print("=" * 80)
    ...
    db_model, db_feats = joblib.load(os.path.join(DOUBAO_MODEL_DIR, 'doubao_t1t2_model.joblib'))

    strategies = {}

    for sname, sel_type in [('doubao_result', 's1'), ('NewIdea S2', 's2'), ('NewIdea S3', 's3')]:
        print(f"\n  Backtesting {sname}...")
        trades = []
        for idx in range(1, len(all_test_dates) - 2):
            ...
            X = df[db_feats].fillna(0)
            if len(X) == 0:
                continue
            df['prob'] = db_model.predict_proba(X)[:, 1]
        ...
        eq, stats = run_backtest(td, all_dates_set)
    ...
    ax.set_title('Strategy Comparison (Same Model, Different Selection, No TP)', fontsize=13, fontweight='bold')
