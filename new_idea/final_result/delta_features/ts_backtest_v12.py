"""
TS Enhanced Backtest v12 - Based on correct trades.csv (356 trades, Sharpe=5.49)
Add TS features and test filtering/reranking schemes.
"""
import os, gc, time
import pandas as pd
import numpy as np
...
def load_prev_data_batch(ts_codes, prev_dates):
    """Load prev data for multiple stocks at once."""
    result = {}
...
def main():
    print("=" * 90, flush=True)
    print("  TS Enhanced Backtest v12 - Based on correct trades.csv", flush=True)
    print("=" * 90, flush=True)
    ...
    print("\n[Step 1] Computing TS features...", flush=True)

    all_ts_codes = set(trades['ts_code'].unique())
    all_date_ts = set(trades['date_t'].unique())

    prev_cache = {}
    for d_t in sorted(all_date_ts):
        idx = all_dates_idx.get(str(d_t), -1)
        if idx < 5:
            continue
        prev_dates = [all_dates[idx - j] for j in range(1, 6)]
        prev_cache[str(d_t)] = load_prev_data_batch(all_ts_codes, prev_dates)
        gc.collect()

    print(f"  Cached {len(prev_cache)} dates", flush=True)
    ...
    for i, (_, trade) in enumerate(trades.iterrows()):
        ...
        if date_t in prev_cache:
            pc = prev_cache[date_t]
            prev_dates_sorted = sorted(pc.keys(), reverse=True)

            if len(prev_dates_sorted) >= 1:
                d1 = prev_dates_sorted[0]
                if ts_code in pc[d1]:
                    p = pc[d1][ts_code]
                    c1 = p.get('close', 0)
                    if c1 > 0:
            ...
            if len(prev_dates_sorted) >= 5:
                c5_date = prev_dates_sorted[4]
                if ts_code in pc[c5_date]:
                    c5 = pc[c5_date]['close']
                    if c5 > 0:
                ...
                cs, vs, cnt = 0.0, 0.0, 0
                for j in range(5):
                    d = prev_dates_sorted[j]
                    if ts_code in pc[d]:
                        cs += pc[d][ts_code]['close']
                        vs += pc[d][ts_code]['vol']
                        cnt += 1
                if cnt >= 3:
                    ma5 = cs / cnt
        ...
        vol_ratio = (trade.get('buy_price', 0) / (vol_mean_5 + 1e-8)) if vol_mean_5 > 0 else 0
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(THIS_DIR, 'ts_backtest_v12_comparison.png'), dpi=150, bbox_inches='tight')
    ...
    for sname, (eq, stats, tdf) in sorted_r[:5]:
        if not eq.empty:
            eq.to_csv(os.path.join(THIS_DIR, f'equity_ts_v12_{sname}.csv'), index=False)
