"""
Feature Analysis v2 - Only load test period data to save memory.
Uses the corrected labeled panel (2.96M rows).
"""
import os, sys
import pandas as pd
import numpy as np
...
def main():
    print("=" * 90, flush=True)
    print("  TS Feature Analysis v2 - Memory Optimized", flush=True)
    print("=" * 90, flush=True)

    # Load only test period data
    print("\n[Step 1] Loading labeled panel (test period only)...", flush=True)
    labeled_path = os.path.join(THIS_DIR, 'ts_panel_labeled.parquet')

    # Load full panel then filter
    full_df = pd.read_parquet(labeled_path)
    test_df = full_df[full_df['date'] >= int(TEST_START)].copy()
    del full_df
    print(f"  Test period: {len(test_df)} rows, pos_rate={test_df['label'].mean():.3f}", flush=True)

    # Feature Analysis
    print("\n[Step 2] Feature Analysis...", flush=True)
