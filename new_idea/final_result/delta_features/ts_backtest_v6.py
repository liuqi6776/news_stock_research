"""
TS Enhanced Backtest v6 - Dict-based lookups instead of merge.
...
def load_day_dict(d_int):
    """Load day data as dict of dicts: {ts_code: {col: val, ...}}"""
    d = str(d_int)
...
def compute_ts_features_dict(current_df, prev_dfs):
    """Compute TS features using DataFrame set_index lookups.
    prev_dfs: list of DataFrames with ts_code as index.
    """
    n = len(current_df)
    ...
    # Pre-extract columns from prev_dfs for vectorized lookup
    def get_prev_col(prev_df, col, ts_codes):
        """Get column values for ts_codes from prev_df (indexed by ts_code)."""
        try:
            vals = prev_df.reindex(ts_codes)[col].values
        except KeyError:
            vals = np.full(len(ts_codes), np.nan)
        return vals

    # 1-day features
    if len(prev_dfs) >= 1:
        c1 = get_prev_col(prev_dfs[0], 'close', ts_codes)
        v1 = get_prev_col(prev_dfs[0], 'vol', ts_codes)
        cc1 = get_prev_col(prev_dfs[0], 'chip_concentration', ts_codes)
        wr1 = get_prev_col(prev_dfs[0], 'winner_rate', ts_codes)
        hr1 = get_prev_col(prev_dfs[0], 'hot_rank_pct', ts_codes)
        tr1 = get_prev_col(prev_dfs[0], 'turnover_rate', ts_codes)

        mask = c1 > 0
        ret_1d[mask] = close_vals[mask] / c1[mask] - 1
        log_ret_1d[mask] = np.log(close_vals[mask] / c1[mask])
        mask_v = v1 > 0
        delta_vol_1d[mask_v] = vol_vals[mask_v] / v1[mask_v] - 1
        delta_chip_conc_1d = chip_conc_vals - np.nan_to_num(cc1, nan=chip_conc_vals)
        delta_winner_rate_1d = winner_rate_vals - np.nan_to_num(wr1, nan=winner_rate_vals)
        delta_hot_rank_1d = hot_rank_vals - np.nan_to_num(hr1, nan=hot_rank_vals)
        delta_turnover_1d = turnover_vals - np.nan_to_num(tr1, nan=turnover_vals)

    # 2-day features
    if len(prev_dfs) >= 2:
        c2 = get_prev_col(prev_dfs[1], 'close', ts_codes)
        mask = c2 > 0
        ret_2d[mask] = close_vals[mask] / c2[mask] - 1
        log_ret_2d[mask] = np.log(close_vals[mask] / c2[mask])

    # 5-day features
    if len(prev_dfs) >= 5:
        c5 = get_prev_col(prev_dfs[4], 'close', ts_codes)
        mask = c5 > 0
        ret_5d[mask] = close_vals[mask] / c5[mask] - 1

        vols = np.stack([get_prev_col(prev_dfs[j], 'vol', ts_codes) for j in range(5)], axis=1)
        closes = np.stack([get_prev_col(prev_dfs[j], 'close', ts_codes) for j in range(5)], axis=1)
        vols = np.nan_to_num(vols, nan=0)
        closes = np.nan_to_num(closes, nan=0)
        vol_mean_5 = vols.mean(axis=1)
        ma5 = closes.mean(axis=1)

    # 10-day features
    if len(prev_dfs) >= 10:
        c10 = get_prev_col(prev_dfs[9], 'close', ts_codes)
        mask = c10 > 0
        ret_10d[mask] = close_vals[mask] / c10[mask] - 1

        closes10 = np.stack([get_prev_col(prev_dfs[j], 'close', ts_codes) for j in range(10)], axis=1)
        closes10 = np.nan_to_num(closes10, nan=0)
        ma10 = closes10.mean(axis=1)

    # RSI
    if len(prev_dfs) >= 14:
        closes14 = np.stack([get_prev_col(prev_dfs[j], 'close', ts_codes) for j in range(14)], axis=1)
        closes14 = np.nan_to_num(closes14, nan=0)
        diffs = np.diff(closes14, axis=1)
    ...
    if 'ret_5d' in df.columns:
        df['ret_accel'] = df['ret_1d'] - df['ret_5d'] / 5.0
...
def main():
    print("=" * 90, flush=True)
    print("  TS Enhanced Backtest v6 - Dict-based Lookups", flush=True)
    print("=" * 90, flush=True)
    ...
    trades_path = os.path.join(THIS_DIR, 'pool_ts_v6.csv')
    ...
    if os.path.exists(trades_path):
        ...
        for i, (idx, d_t) in enumerate(test_dates):
            try:
                ...
                current = load_day_dict(d_t_int)
                if current is None:
                    continue
                current['date'] = d_t_int

                # Load previous days as DataFrames (use set_index for fast lookup)
                prev_dfs = []
                for j in range(1, min(LOOKBACK + 1, idx + 1)):
                    ...
                    if prev_d not in prev_cache:
                        prev_data = load_day_dict(int(prev_d))
                        if prev_data is not None:
                            prev_data = prev_data.set_index('ts_code')
                ...
                # Compute TS features using dict lookups
                current = compute_ts_features_dict(current, prev_dfs)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(THIS_DIR, 'ts_backtest_v6_comparison.png'), dpi=150, bbox_inches='tight')
    ...
    for sname, (eq, stats, tdf) in sorted_r[:5]:
        if not eq.empty:
            eq.to_csv(os.path.join(THIS_DIR, f'equity_ts_v6_{sname}.csv'), index=False)
