...
def main():
    print("DELTA FEATURES - Trade Pool Generation & Backtest", flush=True)
    ...
    print(f"Delta model loaded", flush=True)
    ...
    if os.path.exists(trades_path):
        ...
        print("Generating trade pool...", flush=True)
        all_picks = []
        count = 0
        ...
        for i, (idx, d_t) in enumerate(test_dates):
            ...
            try:
                ...
                print(f"  ERROR loading {d_t}: {e}", flush=True)
                continue
            if df is None:
                continue
            ...
            batch = valid[['ts_code', 'prob', 'delta_winner_rate_1d', 'delta_chip_concentration_1d',
                           'ret_1d', 'ret_3d', 'delta_turnover_rate_1d',
                           'chip_price_diverge', 'vol_price_diverge',
