"""
NewIdea S3 strategy backtest code
Model: doubao_t1t2_model.joblib (same as doubao_result)
Strategy: S3 - score = prob*0.7 + winner_rate_norm*0.2 + (1-chip_conc_norm)*0.1, Top3
Trading Logic: T select -> T+1 buy (open) -> T+2 sell (close, NO take profit)
Label: (T+2 close / T+1 open - 1) > 0.04
circ_mv filter: <= 1000000 (100wan)
"""
import os
import sys
import pandas as pd
...
MODEL_PATH = os.path.join(PARENT_DIR, 'doubao', 'models', 'doubao_t1t2_model.joblib')
...
def backtest(trades_df, all_dates_set):
...
def run():
    ...
    trades = []
    for idx in tqdm(range(1, len(all_dates) - 2)):
        d_t = all_dates[idx]
    ...
    print(f"NewIdea S3 (T+1 buy T+2 close sell, No TP, score composite):")
    print(f"  Total: {stats['total']:.2%}, Annual: {stats['ann']:.2%}, Sharpe: {stats['sharpe']:.2f}, MDD: {stats['mdd']:.2%}")
    print(f"  Trades: {stats['trades']}, Cannot sell: {stats['cannot_sell']}")
    return eq, trades_df
