...
def backtest_method(model, feats, method_name):
    """对单个方法运行修正版回测"""
    print(f"\n{'='*60}")
    print(f"修正回测: {method_name}")
    print(f"{'='*60}")
    ...
    for d in tqdm(test_dates, desc=f"  {method_name}"):
        curr_idx = date_idx[d]
    ...
    print(f"  Skipped: limit_up={skipped_limit_up}, limit_down={skipped_limit_down}, cyb={skipped_cyb}, empty={empty_files}, no_candidates={no_candidates}")
    ...
    # 保存结果
    trades_df.to_csv(os.path.join(THIS_DIR, f"trades_{method_name}_Fixed.csv"), index=False)
    pd.DataFrame({"equity": equity}).to_csv(os.path.join(THIS_DIR, f"equity_{method_name}_Fixed.csv"), index=False)
...
# Baseline
if os.path.exists(os.path.join(THIS_DIR, 'baseline_model.joblib')):
    models['Baseline_5feat'] = (joblib.load(os.path.join(THIS_DIR, 'baseline_model.joblib')), BASE_FEATS)
    print("  Baseline_5feat: loaded")

# Enhanced (best_model_v2)
if os.path.exists(os.path.join(THIS_DIR, 'best_model_v2.joblib')):
    ...
    models['Enhanced_All'] = (model, feats)
    print("  Enhanced_All: loaded")

# Chan
if os.path.exists(os.path.join(THIS_DIR, 'chan_model.joblib')):
    chan_feats = BASE_FEATS + CHAN_FEATS
    models['Chan_Only'] = (joblib.load(os.path.join(THIS_DIR, 'chan_model.joblib')), chan_feats)
    print("  Chan_Only: loaded")

# Lynch
if os.path.exists(os.path.join(THIS_DIR, 'lynch_model.joblib')):
    lynch_feats = BASE_FEATS + LYNCH_FEATS
    models['Lynch_Only'] = (joblib.load(os.path.join(THIS_DIR, 'lynch_model.joblib')), lynch_feats)
    print("  Lynch_Only: loaded")

# Quant
if os.path.exists(os.path.join(THIS_DIR, 'quant_model.joblib')):
    quant_feats = BASE_FEATS + QUANT_FEATS
    models['Quant_Only'] = (joblib.load(os.path.join(THIS_DIR, 'quant_model.joblib')), quant_feats)
    print("  Quant_Only: loaded")

# final_best (使用best_model.joblib)
if os.path.exists(os.path.join(THIS_DIR, 'best_model.joblib')):
    model = joblib.load(os.path.join(THIS_DIR, 'best_model.joblib'))
    feats = list(model.feature_names_in_) if hasattr(model, 'feature_names_in_') else ENHANCED_FEATS
    models['final_best'] = (model, feats)
    print("  final_best: loaded")
...
for name, (model, feats) in models.items():
    trades, equity, stats = backtest_method(model, feats, name)
    if stats:
        results[name] = stats
...
if results:
    ...
    summary.to_csv(os.path.join(THIS_DIR, 'comparison_summary_fixed.csv'))
    print(f"\n汇总已保存至 comparison_summary_fixed.csv")
else:
    print("无有效结果")
