...
for m in methods:
    ...
    print(f"\n{m:15s} | 交易:{len(trades):4d} | 收益>20%:{len(over_20):3d} | 创业板:{len(cyb):3d} | 科创:{len(kcb):3d} | 涨停买:{limit_up:3d} | 跌停卖:{limit_down:3d} | 最高:{trades['ret'].max()*100:6.2f}% | 最低:{trades['ret'].min()*100:6.2f}% | 平均:{trades['ret'].mean()*100:6.2f}%")
