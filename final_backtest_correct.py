...
def main():
    ...
    print("\n" + "="*80)
    print("回测：无止损 + 只对全天最高价=跌停价的股票按 T+3 开盘价卖")
    print("="*80)
    ...
    for date_t2, group in trades_df.groupby('date_t2', sort=True):
        ...
        for _, trade in group.iterrows():
            ...
            if is_cannot_sell:
                ...
                date_t3 = get_next_trading_day(date_t2)
    ...
    print(f"其中 T+2 全天最高价=跌停价（T+3 卖）: {cannot_sell_trades}")
    print(f"总收益: {total_ret:+.2%}, 年化: {ann_ret:+.2%}")
    print(f"夏普: {sharpe:.2f}, 回撤: {mdd:.2%}")
    print(f"交易天数: {len(equity)}, 最终资金: ¥{capital:,.2f}")
    print("="*80)
    
    eq_df.to_csv(os.path.join(OUTPUT_DIR, 'final_backtest_correct_equity.csv'), index=False)
    
    plt.figure(figsize=(16, 10))
    plt.plot(eq_df['date'], eq_df['nav'], label='无止损 + 正确处理跌停', linewidth=2)
    plt.title('策略净值曲线（无止损 + 正确处理跌停）', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'final_backtest_correct.png'), dpi=150)
    print(f"\n净值图已保存: {OUTPUT_DIR}/final_backtest_correct.png")
