...
def run_backtest_with_take_profit(take_profit_pct, test_dates, model, feats, news_market_df, news_stock_sector_df):
    initial_cap = 100000.0
    capital = initial_cap
    equity = []
    
    for i in tqdm(range(len(test_dates)-2), desc=f"止盈 {take_profit_pct*100:.0f}%"):
        d_t = test_dates[i]
        d_t1 = test_dates[i+1]
        d_t2 = test_dates[i+2]
...
def main():
    ...
    for tp in take_profit_list:
        print(f"\n正在回测止盈点: {tp*100:.0f}%")
        result = run_backtest_with_take_profit(tp, test_dates, model, feats, news_market_df, news_stock_sector_df)
        if result:
