...
def main():
    ...
    for take_profit_pct in take_profit_list:
        ...
        print(f"  完成: 总收益 {total_ret:+.2%}, 最终资金 ¥{capital:,.2f}")
    
    print("\n" + "="*80)
    print("回测结果汇总")
    print("="*80)
    df_summary = pd.DataFrame(results)
    df_summary['止盈点'] = (df_summary['take_profit_pct'] * 100).astype(int).astype(str) + '%'
    df_summary['总收益率'] = df_summary['total_ret'].apply(lambda x: f"{x:+.2%}")
    df_summary['最终资金'] = df_summary['final_cap'].apply(lambda x: f"¥{x:,.2f}")
    print(df_summary[['止盈点', '总收益率', '最终资金']].to_string(index=False))
    
    summary_csv = os.path.join(OUTPUT_DIR, 'take_profit_comparison.csv')
    df_summary.to_csv(summary_csv, index=False)
    print(f"\n结果已保存: {summary_csv}")
    
    best = df_summary.loc[df_summary['total_ret'].idxmax()]
    print(f"\n最优结果: 止盈 {best['止盈点']}, 总收益 {best['总收益率']}, 最终资金 {best['最终资金']}")
