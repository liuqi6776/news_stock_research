...
def backtest_strategy_on_trades(trades_df, strategy_name):
    take_profit = 0.08
    ...
    return eq_df, total_ret, ann_ret, sharpe, mdd
...
def main():
    ...
    for strat_id, strat_name in strategies.items():
        print(f"\n策略{strat_id}: {strat_name}")
        eq_df, t_ret, a_ret, sharpe, mdd = backtest_strategy_on_trades(trades_data[strat_id], strat_id)
    ...
    ax.plot(df_doubao['date'], df_doubao['nav'], 
    ...
    for i, (strat_id, strat_name) in enumerate(strategies.items()):
        ...
        ax.plot(eq_df['date'], eq_df['nav'] / eq_df['nav'].iloc[0], 
                 label=f'策略{strat_id} (最终: {(eq_df["nav"].iloc[-1]/eq_df["nav"].iloc[0]):.2f}x)', 
                 linewidth=2, color=colors[i], alpha=0.8)
    
    ax.set_title('四种策略收益曲线对比', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plot_path = os.path.join(OUTPUT_DIR, 'three_strategies_vs_doubao.png')
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write("\n## 回测结果\n\n")
        f.write("| 策略 | 总收益 | 年化 | 夏普 | 最大回撤 |\n")
        f.write("|------|-------|------|------|---------|\n")
        f.write(f"| doubao_result 原策略 | {t_doubao:.2%} | {a_doubao:.2%} | {s_doubao:.2f} | {dd_doubao:.2%} |\n")
        
        for strat_id, strat_name in strategies.items():
            r = backtest_results[strat_id]
            f.write(f"| {strat_name} | {r['total_return']:.2%} | {r['annual_return']:.2%} | {r['sharpe']:.2f} | {r['max_drawdown']:.2%} |\n")
        
        f.write("\n## 收益曲线\n\n")
        f.write("![对比图](three_strategies_vs_doubao.png)\n")
