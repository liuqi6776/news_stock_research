...
def get_prev_trading_days(current_date_int, count):
    days = []
    current_dt = int_to_date(current_date_int)
    i = 1
    while len(days) < count:
        prev_dt = current_dt - timedelta(days=i)
        prev_int = date_to_int(prev_dt)
        if os.path.exists(os.path.join(PRICE_DIR, f"{prev_int}.parquet")):
            days.append(prev_int)
        i += 1
        if i > 60:
            break
    return days
...
def is_new_stock(ts_code, date_t, min_days=10):
    prev_days = get_prev_trading_days(date_t, min_days)
    if len(prev_days) < min_days:
        return True
    
    count = 0
    for day in prev_days:
        df = load_price_data(day)
        if df is None:
            continue
...
def main():
    ...
    print("\n" + "="*80)
    print("回测：无止损 + 连续跌停处理 + 排除新股（数据少于10天）")
    print("="*80)
    ...
    for idx, trade in trades_df.iterrows():
        ...
        if (idx + 1) % 100 == 0:
            print(f"  处理进度: {idx+1}/{len(trades_df)} ({((idx+1)/len(trades_df)*100):.1f}%)")
        
        if is_new_stock(ts_code, date_t):
            excluded_new_stocks += 1
        else:
            filtered_trades.append(trade)
    
    print(f"\n筛选完成！")
    print(f"排除新股: {excluded_new_stocks}")
    print(f"剩余交易数: {len(filtered_trades)}")
    
    filtered_df = pd.DataFrame(filtered_trades)
    
    for date_t2, group in filtered_df.groupby('date_t2', sort=True):
    ...
    print("回测结果（正确筛选新股版）")
    print("="*80)
    print(f"排除新股（数据少于10天）: {excluded_new_stocks}")
    ...
    eq_df.to_csv(os.path.join(OUTPUT_DIR, 'backtest_proper_new_stock_filter_equity.csv'), index=False)
    
    plt.figure(figsize=(16, 10))
    plt.plot(eq_df['date'], eq_df['nav'], label='无止损 + 连续跌停处理 + 正确筛选新股', linewidth=2)
    plt.title('策略净值曲线（正确筛选新股版）', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'backtest_proper_new_stock_filter.png'), dpi=150)
    print(f"\n净值图已保存: {OUTPUT_DIR}/backtest_proper_new_stock_filter.png")
