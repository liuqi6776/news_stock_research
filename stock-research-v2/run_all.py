...
from backtest_cb_scorecard import run_scorecard_backtest
...
def generate_comparison_plots(df_pit):
    ...
    # 3. Scorecard & Melt Managed: N=20, Freq=2W, 15% Melt
    df_nav_scorecard, _, melt_log = run_scorecard_backtest(
        df_pit, N=20, rebalance_freq='2W',
        min_rating='A', min_size=1.0, min_maturity=0.5,
        use_multi_factor=True, use_risk_control=True,
        use_scorecard=True, use_portfolio_melt=True,
        melt_threshold=-0.15, cooldown_days=60
    )
    
    # Print comparison metrics
    print("\n" + "="*80)
    print("                    STRATEGY METRICS COMPARISON (N=20, 2W)")
    print("="*80)
    for name, nav in [("Baseline Double-Low", df_nav_base['nav']),
                      ("Robust Multi-Factor", df_nav_robust['nav']),
                      ("Scorecard & Melt Managed", df_nav_scorecard['nav'])]:
        m = compute_metrics(nav)
        print(f"| {name:25s} | CAGR: {m['CAGR']:.2%} | Sharpe: {m['Sharpe']:.2f} | MaxDD: {m['Max Drawdown']:.2%} | Calmar: {m['Calmar']:.2f} |")
    print("="*80)
    if melt_log:
        print(f"Melt triggers for Scorecard Strategy: {melt_log}")
    
    # 4. Load CSI 1000 Benchmark if available
    df_nav_bench = None
    ...
    # Plot NAV
    ax1.plot(df_nav_scorecard.index, df_nav_scorecard['nav'] / 1e6, label='Scorecard & Melt Managed (20% Budget)', color='#9467bd', linewidth=2.5)
    ax1.plot(df_nav_robust.index, df_nav_robust['nav'] / 1e6, label='Robust Multi-Factor (Filters+Risk+Position)', color='#2ca02c', linewidth=2.0)
    ...
    # Plot Drawdown
    dd_scorecard = (df_nav_scorecard['nav'] - df_nav_scorecard['nav'].cummax()) / df_nav_scorecard['nav'].cummax()
    ...
    ax2.fill_between(df_nav_scorecard.index, dd_scorecard, 0, color='#9467bd', alpha=0.15)
    ax2.plot(df_nav_scorecard.index, dd_scorecard, color='#9467bd', linewidth=1.5, label='Scorecard & Melt')
    
    ax2.plot(df_nav_robust.index, dd_robust, color='#2ca02c', linewidth=1.2, label='Robust Multi-Factor')
    ax2.plot(df_nav_base.index, dd_base, color='#1f77b4', linewidth=1.0, label='Baseline')
