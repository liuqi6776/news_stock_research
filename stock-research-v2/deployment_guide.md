...
在进行任何打分排序前，必须严格过滤以下不合规标的（**从源头杜绝 2024 信用违约与退市暴雷**）：
...
*(注：按消融实验 `cb_ablation_study.csv` 验证，双低因子与溢价率打分已包含溢价保护，硬切溢价率上限会误杀优质债底标的，故不作硬性上限切除。)*
...
## 三、 盘中风控与止损黑名单 / Intraday Risk Management & Blacklist

在非调仓日的日常交易时间（09:30 - 15:00），需盯住 20 只持仓的以下两条硬性红线：

1. **个券 -5% 硬止损 / -5% Stop-Loss Line**:
   * **触发条件**: 当某只持仓转债的最新现价 $\le 0.95 \times \text{买入成本价}$。
   * **标准动作**: 无视任何技术指标，在 5 分钟内按市价/卖一价强制清仓变现。
   * **止损黑名单机制 (Angle 2)**: **止损卖出后的 20 个交易日内，该债券进入黑名单，禁止重新买入**（彻底消除 65.17% 的“止损卖出 $\rightarrow$ 下期又买回 $\rightarrow$ 再次止损”反复打脸损耗）。
2. **130 元高价强赎预警 / 130 CNY Redemption Warning Line**:
   * **触发条件**: 当某只持仓转债现价 $\ge 130.0$ 元。
   * **标准动作**: 锁定超额利润，在 15:00 前全部卖出变现。

---

## 四、 月度调仓标准动作流程 (SOP) / Monthly Rebalance Workflow

**调仓周期**: **每月执行一次 (Angle 5)**（固定在 **每月最后一个交易日的 14:40 - 15:00** 进行）。
* **降本增效成果**: 调仓频率从每年 26 次降至 12 次，**交易摩擦成本与手动操作负担直接减半**。
