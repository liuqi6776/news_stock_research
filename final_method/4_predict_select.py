...
Signal levels (based on backtest):
  STRONG  : prob > 0.8  -> backtest win rate 100%, mean return +12.78%
  MODERATE: 0.6 <= prob < 0.8 -> backtest win rate ~60%, mean return +2.68%
  CASH    : prob < 0.6  -> backtest win rate < 50%, expected return negative
"""
import os
import json
import sys
...
def compute_ts_features(df, d_curr):
    ...
    if os.path.exists(p_prev_chip):
        prev_chip = pd.read_parquet(p_prev_chip,
                                    columns=['ts_code', 'winner_rate', 'cost_85pct', 'cost_15pct', 'cost_50pct'])
...
def select(df, model, feats, use_ts_filter=True):
    ...
    if use_ts_filter and 'ts_score' in picks.columns:
        ...
        if picks.empty:
            ...
            picks = df[(df['news_stock_impact'] > 0) & (df['prob'] >= MIN_PROB_THRESHOLD)].sort_values('prob',
