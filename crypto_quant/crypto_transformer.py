...
时空跨资产关系 Transformer 模型 (时序表征 + 跨币种注意力 + 皮尔逊排序优化)
"""
import torch
...
class TemporalConvEncoder(nn.Module):
    """1D 因果时间卷积编码器，提取单币过去 48 小时的时序动量与波动状态"""
...
class CryptoSTTransformer(nn.Module):
    ...
    """
    def __init__(self, num_assets=4, in_features=23, lookback=12, d_model=64, n_heads=4, num_layers=2, dropout=0.15):
        super().__init__()
        self.num_assets = num_assets
        self.d_model = d_model

        # 1. 单币时序特征编码器 (Temporal Sequence Encoder)
        self.temporal_encoder = TemporalConvEncoder(in_features, d_model, dropout)
...
class CryptoCombinedLoss(nn.Module):
    """
    多任务综合损失函数:
    Pearson IC 排序损失 + Huber 收益拟合损失 + 方向二分类损失
    """
    def __init__(self, alpha_pearson=1.0, beta_huber=15.0, gamma_cls=0.5):
        super().__init__()
    ...
    def forward(self, outputs, target_4h, target_8h):
        ...
        # 2. Huber 收益回归损失
        l_huber = self.huber_loss(pred_4h, target_4h)
...
if __name__ == '__main__':
    ...
    model = CryptoSTTransformer(num_assets=4, in_features=23, lookback=12).to(device)
    criterion = CryptoCombinedLoss()

    dummy_x = torch.randn(16, 4, 12, 23).to(device)
    dummy_y4h = torch.randn(16, 4) * 0.02
    dummy_y8h = torch.randn(16, 4) * 0.03
    dummy_y4h = dummy_y4h.to(device)
    dummy_y8h = dummy_y8h.to(device)

    out = model(dummy_x)
    loss, loss_dict = criterion(out, dummy_y4h, dummy_y8h)
    loss.backward()

    print("Model test passed!")
    print(f"Output pred_4h shape: {out['pred_4h'].shape}")
    print(f"Loss dict: {loss_dict}")
    print("Gradient norm verified.")
