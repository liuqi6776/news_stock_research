...
def is_new_stock(ts_code, date_t, check_days=30):
    date_t_dt = int_to_date(date_t)
    for i in range(1, check_days + 1):
        check_dt = date_t_dt - timedelta(days=i)
        check_int = date_to_int(check_dt)
        df = load_price_data(check_int)
        if df is not None:
...
def main():
    ...
    print(f"排除新上市股票: {excluded_new_stocks}")
    ...
    eq_df.to_csv(os.path.join(OUTPUT_DIR, 'backtest_enhanced_equity.csv'), index=False)
    
    plt.figure(figsize=(16, 10))
    plt.plot(eq_df['date'], eq_df['nav'], label='无止损 + 连续跌停处理 + 排除新上市', linewidth=2)
    plt.title('策略净值曲线（增强版）', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'backtest_enhanced.png'), dpi=150)
    print(f"\n净值图已保存: {OUTPUT_DIR}/backtest_enhanced.png")
