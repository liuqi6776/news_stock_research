...
def is_new_stock_by_code(ts_code):
    if ts_code.startswith('301'):
        return True
    if ts_code.startswith('001'):
        return True
    if ts_code.startswith('605'):
        return True
    if ts_code.startswith('003'):
        return True
    return False
...
def main():
    ...
    for _, trade in trades_df.iterrows():
        ...
        if is_new_stock_by_code(ts_code):
    ...
    eq_df.to_csv(os.path.join(OUTPUT_DIR, 'backtest_final_enhanced_equity.csv'), index=False)
    
    plt.figure(figsize=(16, 10))
    plt.plot(eq_df['date'], eq_df['nav'], label='无止损 + 连续跌停处理 + 排除新股', linewidth=2)
    plt.title('策略净值曲线（最终增强版）', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'backtest_final_enhanced.png'), dpi=150)
    print(f"\n净值图已保存: {OUTPUT_DIR}/backtest_final_enhanced.png")
