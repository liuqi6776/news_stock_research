...
def main():
    ...
    if not os.path.exists(cache_file):
        print("未找到股票缓存文件，请先运行 build_trade_stock_cache.py")
        return
    
    with open(cache_file, 'rb') as f:
        stock_dates = pickle.load(f)
    print(f"已加载 {len(stock_dates)} 只股票的历史数据")
    
    take_profit = 0.08
    
    print("\n" + "="*80)
    print("回测：无止损 + 连续跌停处理 + 正确筛选新股（T日前数据<10天）")
    print("="*80)
    ...
    for _, trade in trades_df.iterrows():
        ...
        if is_new_stock(ts_code, date_t, stock_dates):
    ...
    print("\n" + "="*80)
    print("回测结果（正确筛选新股最终版）")
    print("="*80)
    print(f"排除新股（T日前数据<10天）: {excluded_new_stocks}")
    ...
    eq_df.to_csv(os.path.join(OUTPUT_DIR, 'backtest_correct_new_stock_filter_equity.csv'), index=False)
    
    plt.figure(figsize=(16, 10))
    plt.plot(eq_df['date'], eq_df['nav'], label='无止损 + 连续跌停处理 + 正确筛选新股', linewidth=2)
    plt.title('策略净值曲线（正确筛选新股最终版）', fontsize=16, fontweight='bold')
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, 'backtest_correct_new_stock_filter.png'), dpi=150)
    print(f"\n净值图已保存: {OUTPUT_DIR}/backtest_correct_new_stock_filter.png")
